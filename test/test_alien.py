...
def alien(settings):
    pygame.init()
    screen = pygame.display.set_mode((settings.screen_width, settings.screen_height))
    ai_game = type('MockGame', (object,), {'screen': screen, 'settings': settings})
    alien_instance = Alien(ai_game)
    yield alien_instance
    pygame.quit()
...
def test_alien_initial_position(alien):
    assert alien.rect.x == alien.rect.width, "Alien initial x position should match its width."
    assert alien.rect.y == alien.rect.height, "Alien initial y position should match its height."

def test_alien_moves_right(alien):
    initial_x = alien.x
    alien.update()
    assert alien.x > initial_x, "Alien did not move right after update."

def test_alien_check_edges(alien):
    alien.rect.right = alien.screen.get_rect().right  # Simulate alien at the right edge
    assert alien.check_edges() is True, "Alien should detect the right edge of the screen."

    alien.rect.left = 0  # Simulate alien at the left edge
    assert alien.check_edges() is True, "Alien should detect the left edge of the screen."
